function [y_est,fval] = Cauchy_BFGS_PSO(Rx, Rg, dRg, nu, NPtcl, Nmax, omega_max, omega_min, c1, c2, S)
%Cauchy_BFGS_PSO
...
y_est = pg*scale;
    ...
    function obj = obj_fun(y_vec)
        ...
        for m = 1:M
            for l = 1:L
                obj = obj + log(1 + (Rg(m,l)-norm(y_vec(1:H)-y_vec(H+(m-1)*H+1:H+(m-1)*H+H))-norm(y_vec(1:H)-Rx(:,l)))^2/nu) ...
                    + log(1 + (dRg(m,l)-norm(y_vec(H+(m-1)*H+1:H+(m-1)*H+H)-Rx(:,l)))^2/nu);
            end
        end
